...
#
# Modified July 2010 by jesse dot tane at gmail dot com
# for compatibility with Python 3.x
# Changes to this file:
# Metaclass technique revised
# isWrapperMethod()'s isinstance test fails in 3.x - do we need this?
##
...
MetaMessageType = MessageType('MetaMessageType', (object, ), {})

class Message(MetaMessageType):
    """ Base class of all Message types.

    """
    # left over from Python 2.x version
...
def isWrapperMethod(name, value):
    ...
    return (not name.startswith('_') and
            not name.startswith('error'))
            #and isinstance(value, MethodType))
...
def messageTypeNames():
    ...
    """
    return set([t.typeName for t in list(registry.values())])
